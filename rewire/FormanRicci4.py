...
def _compute_afrc_edges(G: nx.Graph, weight="weight", edge_list=[]) -> dict:
    ...
    for edge in edge_list:
        ...
        edge_afrc[edge] = _compute_afrc_edge(
            G, edge[0], edge[1], num_triangles, num_quadrangles
        )
...
class FormanRicci4:
    ...
    def __init__(self, G: nx.Graph, weight: str = "weight"):
        ...
        for cycle in _simple_cycles(
            self.G.to_directed(), 5
        ):  # Only compute 3 and 4 cycles
            if len(cycle) == 3:
        ...
        for edge in list(self.G.edges()):
            ...
            self.G.edges[edge]["triangles"] = (
                len([cycle for cycle in self.triangles if u in cycle and v in cycle])
                / 2
            )
            self.G.edges[edge]["quadrangles"] = (
                len([cycle for cycle in self.quadrangles if u in cycle and v in cycle])
                / 2
            )
